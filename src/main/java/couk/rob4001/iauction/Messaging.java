...
public class Messaging {
	...
	public static void sendMessage(String messageKey, String playerName,
			Auction auction) {
		if (playerName == null) {
			sendMessage(messageKey, (CommandSender) null, auction);
		} else {
			sendMessage(messageKey, plugin.getServer().getPlayer(playerName),
					auction);
		}
	...
	public static void sendMessage(String messageKey, CommandSender player,
		...
		String startAucitonTax = functions.formatAmount(plugin.getConfig()
				.getDouble("auction-start-tax"));
		...
		if (auction != null) {
			...
			if (auction.getStartingBid() == 0) {
				startingBid = functions.formatAmount(auction
						.getMinBidIncrement());
			} else {
				startingBid = functions.formatAmount(auction.getStartingBid());
			}
			minBidIncrement = functions.formatAmount(auction
					.getMinBidIncrement());

			if (auction.getRemainingTime() >= 60) {
				timeRemaining = textConfig.getString("time-format-minsec");
				timeRemaining = timeRemaining.replace("%s",
						Integer.toString(auction.getRemainingTime() % 60));
				timeRemaining = timeRemaining.replace("%m", Integer
						.toString((auction.getRemainingTime() - (auction
			...
			if (auction.getCurrentBid() != null) {
				...
				currentBid = functions.formatAmount(auction.getCurrentBid()
						.getBidAmount());
				currentMaxBid = functions.formatAmount(auction.getCurrentBid()
						.getMaxBidAmount());
				endAuctionTax = functions.formatAmount((long) Math
						.floor(auction.getCurrentBid().getMaxBidAmount()
								* (plugin.getConfig().getDouble(
										"auction-end-tax-percent") / 100D)));
			} else {
				currentBidder = "noone";
			...
			if (typeLot != null) {
				if (typeLot.getType().getMaxDurability() > 0) {
					...
					durabilityRemaining = decimalFormat
							.format((1 - ((double) typeLot.getDurability() / (double) typeLot
		...
		if (messageList == null || messageList.size() == 0) {
			...
			if (originalMessage == null || originalMessage.length() == 0) {
				...
				messageList = Arrays
						.asList(originalMessage.split("(\r?\n|\r)"));
			}
		}
		...
		for (Iterator<String> i = messageList.iterator(); i.hasNext();) {
			...
			if (messageKey == "auction-info-enchantment") {
				if (auction != null && auction.getLotType() != null) {
					...
					for (Entry<Enchantment, Integer> enchantment : enchantments
							.entrySet()) {
						message = originalMessage.replace("%E",
								items.enchantmentName(enchantment));
						if (player == null) {
		...
		if (plugin.getConfig().getBoolean("log-auctions")) {
			...
			try {
				...
				out = new BufferedWriter(new FileWriter(
						auctionLog.getAbsolutePath(), true));
