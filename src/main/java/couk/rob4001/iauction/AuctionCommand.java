	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		...
		if (suspendAllAuctions) {
			if (args.length == 1 && args[0].equalsIgnoreCase("resume")) {
				if (player != null
						&& !plugin.perms.has(player, "auction.admin")) {
					Messaging.sendMessage("no-permission", sender, null);
					return true;
				}
				// Suspend globally:
				suspendAllAuctions = false;
				Messaging.sendMessage("unsuspension-global", (Player) null,
						null);
				return true;
			}
		...
		if (player != null
				&& iAuction.suspendedUsers.contains(playerName.toLowerCase())) {
		...
		if (cmd.getName().equalsIgnoreCase("auction")) {
			if (args.length > 0) {
				if (args[0].equalsIgnoreCase("reload")) {
					...
					if (!iAuction.suspendedUsers
							.contains(args[1].toLowerCase())) {
						Messaging.sendMessage(
								"unsuspension-user-fail-not-suspended", sender,
								null);
						return true;
					}

					iAuction.suspendedUsers.remove(args[1].toLowerCase());
					functions.saveObject(iAuction.suspendedUsers,
							"suspendedUsers.ser");
					Messaging.sendMessage("unsuspension-user", plugin
							.getServer().getPlayer(args[1]), null);
					Messaging.sendMessage("unsuspension-user-success", sender,
							null);
					...
					if (args.length > 1) {
						// Suspend a player:
						if (iAuction.suspendedUsers.contains(args[1]
								.toLowerCase())) {
							Messaging.sendMessage(
									"suspension-user-fail-already-suspended",
									sender, null);
							return true;
						}

						Player playerToSuspend = plugin.getServer().getPlayer(
								args[1]);

						if (playerToSuspend == null
								|| !playerToSuspend.isOnline()) {
							Messaging.sendMessage(
									"suspension-user-fail-is-offline", sender,
									null);
							return true;
						}
						...
						if (plugin.perms.has(playerToSuspend, "auction.admin")) {
							Messaging.sendMessage(
									"suspension-user-fail-is-admin", sender,
									null);
							return true;
						}
						...
						functions.saveObject(iAuction.suspendedUsers,
								"suspendedUsers.ser");
						Messaging.sendMessage("suspension-user",
								playerToSuspend, null);
						Messaging.sendMessage("suspension-user-success",
								sender, null);
					...
					Messaging.sendMessage("suspension-global", (Player) null,
							null);
					...
					// Start new auction!
					if (player == null) {
						Messaging.sendMessage("auction-fail-console", sender,
								null);
						return true;
					}
					if (!plugin.getConfig().getBoolean(
							"allow-gamemode-creative")
							&& player.getGameMode() == GameMode.CREATIVE) {
						Messaging.sendMessage("auction-fail-gamemode-creative",
								sender, null);
						return true;
					}
					...
					plugin.getScope(player).queueAuction(
							new Auction(plugin, player, args, scope), player,
							auction);
					...
					if (auction == null) {
						Messaging.sendMessage("auction-fail-no-auction-exists",
								sender, auction);
						return true;
					}
					if (player == null
						...
						if (plugin.getConfig().getInt(
								"cancel-prevention-seconds") > auction
								.getRemainingTime()
								|| plugin.getConfig().getInt(
										"cancel-prevention-percent") > (double) auction
										.getRemainingTime()
										/ (double) auction.getTotalTime()
										* 100D) {
							Messaging.sendMessage(
									"auction-fail-cancel-prevention", player,
									auction);
						} else {
					...
					if (!plugin.getConfig().getBoolean("allow-early-end")) {
						Messaging.sendMessage("auction-fail-no-early-end",
								player, auction);
						return true;
					}
					...
					return true;
				} else if (args[0].equalsIgnoreCase("stfu")
					...
					if (auction == null) {
						Messaging.sendMessage("auction-info-no-auction",
								player, auction);
						return true;
					}
					auction.info(sender);
			...
			if (player == null) {
				Messaging.sendMessage("bid-fail-console", plugin.getServer()
						.getConsoleSender(), null);
				return true;
			}
			if (!plugin.getConfig().getBoolean("allow-gamemode-creative")
					&& player.getGameMode().equals(GameMode.CREATIVE)) {
				Messaging.sendMessage("bid-fail-gamemode-creative", sender,
						null);
				return true;
			}
