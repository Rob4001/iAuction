...
public class AuctionCommand implements CommandExecutor {
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label,
		...
		if (main.equalsIgnoreCase("start") || main.equalsIgnoreCase("s")) {
			...
			if (iAuction.getInstance().lots.containsKey(player.getName())) {
			...
			if (time < iAuction.getInstance().getConfig()
					.getInt("start.timelow")) {
				Messaging.playerMessage(player, "start.timelow");
				return true;
			}
			if (time > iAuction.getInstance().getConfig()
					.getInt("start.timehigh")) {
				Messaging.playerMessage(player, "start.timehigh");
				return true;
			}
			if (price < iAuction.getInstance().getConfig()
					.getInt("start.pricelow")
					&& !(-1 == iAuction.getInstance().getConfig()
							.getInt("start.pricelow"))) {
				Messaging.playerMessage(player, "start.pricelow");
				return true;
			}
			if (price > iAuction.getInstance().getConfig()
					.getInt("start.pricehigh")
					&& !(-1 == iAuction.getInstance().getConfig()
							.getInt("start.pricehigh"))) {
			...
			((Player) sender)
					.openInventory(new BasicNewInterface(new BasicNewLayout(
							iAuction.getInstance().getConfig()
									.getInt("start.rows")), price, time)
							.getInventory());
			return true;
		}
		...
		if (main.equalsIgnoreCase("list") || main.equalsIgnoreCase("l")) {
			...
			while (it.hasNext()) {
				i++;
				Auction au = it.next();
				Messaging.playerMessage(player, "auction.info", i.toString(),
						au.getOwner().getDisplayName(), au.getTime(),
						au.getBid());
			}
			return true;
		}
		...
		if (main.equalsIgnoreCase("mute")) {
			...
			if (iAuction.getInstance().listeners.contains(player.getName())) {
