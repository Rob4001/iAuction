...
public class InventoryManager {
	...
	public ItemStack quantify(ItemStack type, boolean meta, boolean durability) {
		...
		for (Map.Entry<Integer, ? extends ItemStack> which : _inv.all(
				type.getType()).entrySet())
			if (hasSameEnchantments(type, which.getValue())
					&& isSplash(type.getDurability()) == isSplash(which
							.getValue().getDurability())
					&& ((type.getType().getMaxDurability() <= 0) ? ((!meta) || (handleData(type
							.getData()) == handleData(which.getValue()
							.getDurability()))) : ((!durability) || (type
							.getDurability() == which.getValue()
							.getDurability()))))
				amount += which.getValue().getAmount();

		return new ItemStack(type.getType(), amount, type.getDurability(),
				handleData(type.getData()));
	}
	...
	public boolean contains(ItemStack stack, boolean meta, boolean durability) {
		return (((stack.getAmount() > 0) && (this.quantify(stack, meta,
				durability).getAmount() >= stack.getAmount())) || ((stack
				.getAmount() == 0) && (this.quantify(stack, meta, durability)
				.getAmount() >= 1)));
	}
	...
	public ItemStack addItem(ItemStack stack) {
		...
		for (int i = 0; i < size; i++) {
			...
			if ((amt < max)
					&& (((amt < 1) || ((stack.getTypeId() == slot.getTypeId())
							&& (handleData(stack.getData()) == handleData(slot
									.getDurability())) && (hasSameEnchantments(
								stack, slot)))))) {
				ItemStack tmp = new ItemStack(stack.getType(),
						(((amount + amt) > max) ? (max) : (amount + amt)),
						stack.getDurability(), handleData(stack.getData()));
		...
		if ((amount > 0) && ((_player != null) || (_loc != null)))
			for (; amount > 0; amount -= max)
				((_player != null) ? (_player.getLocation()) : (_loc))
						.getWorld().dropItemNaturally(
								((_player != null) ? (_player.getLocation())
										: (_loc)),
								addEnchs(
										new ItemStack(stack.getType(),
												((amount > max) ? (max)
														: (amount)), stack
														.getDurability(),
												handleData(stack.getData())),
										stack));
		else
			return addEnchs(
					new ItemStack(stack.getType(), amount, stack
							.getDurability(), handleData(stack.getData())),
					stack);

		return addEnchs(new ItemStack(stack.getType(), 0,
				stack.getDurability(), handleData(stack.getData())), stack);
	}
	...
	public ItemStack remove(ItemStack stack, boolean meta, boolean durability) {
		...
		for (Map.Entry<Integer, ? extends ItemStack> which : _inv.all(
				stack.getType()).entrySet()) {
			if (hasSameEnchantments(stack, which.getValue())
					&& isSplash(stack.getDurability()) == isSplash(which
							.getValue().getDurability())
					&& ((stack.getType().getMaxDurability() <= 0) ? ((!meta) || (handleData(stack
							.getData()) == handleData(which.getValue()
							.getDurability()))) : ((!durability) || (stack
							.getDurability() == which.getValue()
							.getDurability())))) {
		...
		return new ItemStack(stack.getType(), (stack.getAmount() - amount),
				stack.getDurability(), handleData(stack.getData()));
	}

	private boolean hasSameEnchantments(ItemStack stack, ItemStack value) {
		if ((stack.getEnchantments().isEmpty())
				&& (value.getEnchantments().isEmpty()))
			return true;
		if ((!stack.getEnchantments().isEmpty())
				&& (value.getEnchantments().isEmpty()))
			return false;
		if ((stack.getEnchantments().isEmpty())
				&& (!value.getEnchantments().isEmpty()))
			return false;
		for (Entry<Enchantment, Integer> ench : stack.getEnchantments()
				.entrySet()) {
			boolean found = false;
			for (Entry<Enchantment, Integer> compare : value.getEnchantments()
					.entrySet()) {
				if (ench.getKey() == compare.getKey()) {
					if (ench.getValue() == compare.getValue())
						found = true;
				}
			}
			if (found == false)
		...
		for (Entry<Enchantment, Integer> ench : value.getEnchantments()
				.entrySet()) {
			boolean found = false;
			for (Entry<Enchantment, Integer> compare : stack.getEnchantments()
					.entrySet()) {
				if (ench.getKey() == compare.getKey()) {
					if (ench.getValue() == compare.getValue())
						found = true;
